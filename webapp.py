...
def main():
    ...
    psnpxg_per_shot_on_target_against = st.number_input('Quality of shoots(xG) that goalkeeper faced (between 0-1)', min_value=0.0,max_value=1.0, value=0.0)
    clean_sheets = st.number_input('Clean Sheets', min_value=0, value=0)
    goals = st.number_input('Goals', min_value=0, value=0)
    xg_xa_per90 = st.number_input('xG + xA per 90 minutes (between 0-1)', min_value=0.0,max_value=1.0, value=0.0)
